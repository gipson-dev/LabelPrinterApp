...
| 3 | ZPL generator | Complete | Generates escaped ZPL for text, Code 128, Code 39, QR, Line, Box, media mode, orientation, margins, darkness, speed, wrapping, alignment, bold, underline, auto-fit, and corrected normal-text vertical origin. |
...
| 5 | Template storage | Complete | JSON load/save via nlohmann/json with fallback default templates plus saved Line/Box, print, and lock flags. |
| 6 | UI wiring | Complete | Qt MainWindow wires menus, toolbars, editor commands, preview, property editor pages, template commands, Excel/CSV table, settings, help, and printing. |
...
| 9 | Better preview | Complete | Scaled preview with rulers, grid, label boundary, printable margin, selection handles, cursor position, vertically centered text, text/barcode/QR/shape rendering, and locked-element drag blocking. |
...
| 16 | Drag-and-drop designer | Complete | Preview supports drag-to-move, side/corner resize handles, layer order, align buttons, equal spacing, snap toggles, cut/copy/paste, undo/redo, zoom fit/in/out, and lock/unlock. |
...
| 19 | Testing and calibration | Started | Core tests cover ZPL, text origin behavior, CSV, variables, serial ranges, and template storage. Manual printer calibration notes remain next. |
| 20 | Final polish | In progress | Classic UI polish, readable controls, functional inspector pages, layout controls, persistent app settings, vertical print/preview alignment, and package rebuilds are ongoing. |
...
- Line and Box elements with preview, resize, template storage, and ZPL output.
- Classic editor commands for Cut, Copy, Paste, Undo, Redo, Zoom In/Out/Fit, and Help.
- Vertically centered preview text plus corrected Zebra text-origin output for closer print matching.
...
3. Continue designer polish with image/logo support, keyboard resize nudging, and finer printer calibration controls if physical test labels show remaining offset.
